...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@""] style:UIBarButtonItemStylePlain target:self action:@selector(leftReturnClick:)];
    ...
    _stepView = [[XFStepView alloc] initWithFrame:CGRectMake(0, 10*DISTANCE_HEIGHT, K_WIDTH, 60*DISTANCE_HEIGHT) Titles:[NSArray arrayWithObjects:@"基本信息", @"身份证", @"个人照片", @"个人简介", nil] WithNum:1];
...
- (void)createCardViewUI{
    ...
    UIImage *image = [UIImage imageNamed:@"apply_open_shop_idcard_front"];
//    图片的高度和宽度
    CGFloat imageWidth = K_WIDTH - 180*DISTANCE_WIDTH;
...
//    配置imageV和label的数据源
    NSArray *imageS = @[@"apply_open_shop_idcard_front",@"apply_open_shop_idcard_rear"];
    NSArray *labS = @[@"身份证正面",@"身份证反面"];
    for (int i = 0; i < 2; i ++) {
        UIImageView *imageV = [[UIImageView alloc] init];
        imageV.frame = CGRectMake(90*DISTANCE_WIDTH, 25*DISTANCE_HEIGHT + (imageHeight + labHeight + (10 + 25)*DISTANCE_HEIGHT)*i, imageWidth, imageHeight);
...
//        给imageV增加手势点击方法
        ...
        [imageV addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imageVTapClick:)]];

        if (i == 0) {
            
           NSData *imageData = [[NSUserDefaults standardUserDefaults]objectForKey:@"frontPic"];
            ...
            self.frontImgV = imageV;
        }else{
            
            NSData *imageData = [[NSUserDefaults standardUserDefaults]objectForKey:@"backPic"];
            
            if (imageData != nil) {
                imageV.image = [UIImage imageWithData:imageData];
            }
            
            self.backImgV = imageV;
        }
        
        
        UILabel *label = [[UILabel alloc] init];
        label.frame = CGRectMake((K_WIDTH - labWidth)/2, 25*DISTANCE_HEIGHT + imageHeight + 10*DISTANCE_HEIGHT + (labHeight+ imageHeight + (25 + 10)*DISTANCE_HEIGHT)*i, labWidth, labHeight);
        label.text = labS[i];
        ...
        if (i == 1) {
            ...
            bgViewHeight = CGRectGetMaxY(label.frame) + 20*DISTANCE_HEIGHT;
    ...
    annotationLab.text = @"*身份证（仅用作认证，不会向用户展示）";
...
- (NSMutableArray *)imageViews{
    if (!_imageViews) {
        _imageViews = [NSMutableArray arrayWithCapacity:2];
    }
    return _imageViews;
}
...
//        NSInteger num = imageView.tag;
//        switch (num) {
//            case 1000:{
//                
//                if ([UIImagePNGRepresentation(imageView.image) isEqual:UIImagePNGRepresentation([UIImage imageNamed:@"apply_open_shop_idcard_front"])]) {
//                    
//                    NSString *tips = [NSString stringWithFormat:@"请输入身份证件的正反两面"];
//                    
...
//            case 1001:{
//                
//                if ([UIImagePNGRepresentation(imageView.image) isEqual:UIImagePNGRepresentation([UIImage imageNamed:@"apply_open_shop_idcard_rear"])]) {
//                    
//                    NSString *tips = [NSString stringWithFormat:@"请输入身份证件的正反两面"];
//                    
//                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Tips" message:tips preferredStyle:UIAlertControllerStyleAlert];
...
//    }
//    
    PersonalPhotoVC *perPhVC = [[PersonalPhotoVC alloc] init];
    
    [self.navigationController pushViewController:perPhVC animated:YES];
...
- (void)imageVTapClick:(UITapGestureRecognizer *)recognizer{
    
    UIView *viewClicked = [recognizer view];
    
    NSInteger tag = viewClicked.tag;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (self.num == 1000) {
        self.frontImgV.image = image;
        NSData *imageData = UIImageJPEGRepresentation(image, 1);
        [[NSUserDefaults standardUserDefaults] setObject:imageData forKey:@"frontPic"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        
    }else{
        NSData *imageData = UIImageJPEGRepresentation(image, 1);
        [[NSUserDefaults standardUserDefaults] setObject:imageData forKey:@"backPic"];
        [[NSUserDefaults standardUserDefaults] synchronize];
