...
@interface DetailedClassificationViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
//    NSMutableArray *eight = [NSMutableArray array];
    ...
    NSString *allCropsClassStr = [NSString stringWithFormat:TOM_HOST,@"api",@"Weiba",@"get_all_weiba_cate",oauth_token,oauth_token_secret];
    NSString *encode = [allCropsClassStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];

    
    
    
    [Q_AHttpRequestManager getAllCropsWithUrl:encode withBlock:^(NSArray *array) {
...
//        NSLog(@"%lu",(unsigned long)_cropClassification.count);
        ...
        NSString *everyCropStr = [NSString stringWithFormat:TOM_HOST_CROP_CID,@"api",@"Weiba",@"get_all_weiba_cate",oauth_token,oauth_token_secret,@"0"];
        NSString *encodeEvery = [everyCropStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        
        NSLog(@"%@",everyCropStr);
        [Q_AHttpRequestManager getDesignatedCropsWithUrl:encodeEvery withBlock:^(NSArray *array) {
...
//    NSLog(@"ddddddddd");
    if ([self.dataSourceDelegate respondsToSelector:@selector(sendDataSource:)]) {
        [self.dataSourceDelegate sendDataSource:self.selectionArray];
    }
...
- (void)changeTitleInDataSourceWith:(NSIndexPath *)indexPath{
    ...
    if (detailSPobj.following == 0) {
        ...
        urlStr = [NSString stringWithFormat:TOM_HOST_CROP_ID,@"api",@"Weiba",@"create",oauth_token,oauth_token_secret,(int)detailSPobj.weiba_id];
...
//        urlStr = [NSString stringWithFormat:@"http://192.168.1.86/thinksns_v3.0/index.php?app=api&mod=Weiba&act=create&oauth_token=988b491a22040ef7634eb5b8f52e0986&oauth_token_secret=2a3d67f5f7bb03035e619518b364912e&id=%d",(int)detailSPobj.weiba_id];
        ...
        _num --;
        [self.follow_destroyArr addObject:detailSPobj];
        ...
        urlStr = [NSString stringWithFormat:TOM_HOST_CROP_ID,@"api",@"Weiba",@"destroy",oauth_token,oauth_token_secret,(int)detailSPobj.weiba_id];
//        urlStr = [NSString stringWithFormat:@"http://192.168.1.86/thinksns_v3.0/index.php?app=api&mod=Weiba&act=destroy&oauth_token=988b491a22040ef7634eb5b8f52e0986&oauth_token_secret=2a3d67f5f7bb03035e619518b364912e&id=%d",(int)detailSPobj.weiba_id];
    ...
    NSLog(@"          关注或者取消关注的作物id      %ld",detailSPobj.weiba_id);
    
    
    NSString *encode = [urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    
    [Q_AHttpRequestManager followCreateOrDestoryWithUrl:encode WithBlock:^(NSArray *array) {
